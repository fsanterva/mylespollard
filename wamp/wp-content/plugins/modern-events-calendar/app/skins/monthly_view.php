...
class MEC_skin_monthly_view extends MEC_skins
{
    ...
    public function day_label($time)
    {
        ...
        {
            return '<h6 class="mec-table-side-title">'.sprintf(__('Events for %s %s', 'mec'), '<span class="mec-color mec-table-side-day"> '.$date_suffix.'</span>', $this->main->date_i18n('F', $time)).'</h6>';
        }
        else return '<h6 class="mec-table-side-title">'.sprintf(__('Events for %s', 'mec'), $this->main->date_i18n('F', $time)).'</h6><h3 class="mec-color mec-table-side-day"> '.$date_suffix.'</h3>';
    }
}