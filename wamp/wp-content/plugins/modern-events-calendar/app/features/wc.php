...
class MEC_feature_wc extends MEC_base
{
    ...
    {
        if($meta->key == 'mec_event_id') $display_key = __('Event', 'mec');
        elseif($meta->key == 'mec_date') $display_key = __('Date', 'mec');
        elseif($meta->key == 'mec_transaction_id') $display_key = __('Transaction ID', 'mec');
    ...
    public function display_value($display_value, $meta)
    {
        ...
        elseif($meta->key == 'mec_date')
        {
            ...
            $display_value = sprintf(__('%s to %s', 'mec'), $start_datetime, $end_datetime);
        }
    ...
    public function validate($data, $errors)
    {
        ...
        foreach($items as $key => $item)
        {
            ...
            {
                if($availability[$ticket_id] == '0') $errors->add('validation', sprintf(__('%s ticket is sold out!', 'mec'), $tickets[$ticket_id]['name']));
                else $errors->add('validation', sprintf(__('Only %s slots remained for %s ticket so you cannot book %s ones.', 'mec'), $availability[$ticket_id], $tickets[$ticket_id]['name'], $quantity));
            }
        }
    }
}