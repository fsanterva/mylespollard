<?php if ( ! function_exists( 'seniman_comment' ) ) :

function seniman_comment( $comment, $args, $depth ) {
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment-body clearfix">
			...
			<div class="meta-comment">
                <div class="comment-author vcard">
                	<?php printf( __( '%s', 'seniman' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>

                	<time datetime="<?php comment_time( 'c' ); ?>">
						<?php printf( _x( '%1$s', '1: date', 'seniman' ), get_comment_date()); ?>
					</time>
                </div><!-- .comment-author .vcard -->
                
				<?php if ( $comment->comment_approved == '0' ) : ?>
                	<div class="waiting-moderation">
					<em><?php esc_html_e( 'Your comment is awaiting moderation.', 'seniman' ); ?></em>
					</div>
				<?php endif; ?>
				...
				<div class="comment-action">
	            	<span class="edit-comment-link"><?php edit_comment_link( esc_html__( 'Edit', 'seniman' ), '<span class="edit-link">', '<span>' ); ?></span>
